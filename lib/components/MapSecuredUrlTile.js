...
const propTypes = {
  ...
  /**
   * The accessToken that is send alongside the request
   */
  accessToken: PropTypes.string.isRequired,
};

class MapSecuredUrlTile extends React.Component {
  render() {
    const AIRMapSecuredUrlTile = this.getAirComponent();
    return <AIRMapSecuredUrlTile {...this.props} />;
  }
}

MapSecuredUrlTile.propTypes = propTypes;

export default decorateMapComponent(MapSecuredUrlTile, {
  componentType: 'SecuredUrlTile',
  providers: {
    google: {
      ios: SUPPORTED,
